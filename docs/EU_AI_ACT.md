...
| **Article 15 — Accuracy, robustness, cybersecurity** | Documented accuracy levels appropriate for the purpose; robustness to input errors and adversarial manipulation; cybersecurity; performance monitored over time | Metrics persisted in `models/metrics.json` and published in `README.md`; CI regenerates metrics deterministically; tests validate schema and domain-level sanity; `UnknownCategoryError` returns HTTP 422 for unseen categorical values; Pydantic validates individual field ranges | **The model collapses on an unseen city.** On a real PDOK BAG held-out-city split (train: Gouda/Rotterdam/Zaanstad, test: Dordrecht), accuracy is **0.092** (macro F1 0.142) — well below the 4-class uniform baseline of 0.250. Three confounds compound: (1) `soil_type` is perfectly confounded with municipality (peat belt vs. sandy clay) — all 974 Dordrecht rows have soil_type=NaN; (2) zero `low`-class training examples — the rule engine produces no low-risk buildings in peat-zone cities; (3) class distribution inversion — training is 76% critical, Dordrecht is 65% moderate. A system that degenerates this far below random on a routine geographic change does not meet the robustness expectation of Article 15. Additionally: accuracy on the random split (0.824) measures rule-recovery from a synthetic label function, not real-world performance; Pydantic does **not** validate cross-field consistency — `{"year_built": 2020, "building_age": 300}` is accepted; no adversarial robustness testing; no security audit of the API | Real soil and hydrology data via PDOK BRO / BodemDalingsKaart / KNMI APIs (breaks the municipality-soil confound); derive `building_age` server-side from `year_built`; add cross-field validation; benchmark against real KCAF data; adversarial input testing; penetration test before any public deployment |
...
- **Article 15:** The model collapses on an unseen city (0.092 accuracy, well below 4-class uniform random 0.250) due to three compounding confounds: soil-type/municipality confound, zero low-class training examples in peat cities, and class-distribution inversion. A system that degenerates this far below random on a routine geographic change does not meet the robustness expectation. Accuracy on the random split measures rule-recovery; cross-field validation is absent.
