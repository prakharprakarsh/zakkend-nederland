...
| **Article 15 — Accuracy, robustness, cybersecurity** | Documented accuracy levels appropriate for the purpose; robustness to input errors and adversarial manipulation; cybersecurity; performance monitored over time | Metrics persisted in `models/metrics.json` and published in `README.md`; CI regenerates metrics deterministically; tests validate schema and domain-level sanity; `UnknownCategoryError` returns HTTP 422 for unseen categorical values; Pydantic validates individual field ranges | Accuracy measures rule-recovery only, not real-world performance; Pydantic does **not** validate cross-field consistency — `{"year_built": 2020, "building_age": 300, ...}` is accepted as valid and returns HTTP 200; no adversarial robustness testing; no security audit of the API | Derive `building_age` server-side from `year_built` and remove it from `BuildingInput`; add cross-field validation; benchmark accuracy against real KCAF data; adversarial input testing; penetration test before any public deployment |
...
- **Article 15:** Accuracy measures rule-recovery; cross-field validation is absent.
