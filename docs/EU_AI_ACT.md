...
- **Article 15:** Held-out-city accuracy improved from **0.092 to 0.614** (macro F1 0.142 → 0.480) after BRO Bodemkaart integration — but the figures are not on the same test labels (soil_type change shifts Dordrecht's class distribution). Majority-class reference: 640/974 = 0.657 — a constant "moderate" predictor outperforms the model's 0.614. `low` class F1 9.8% (14/103 correct, 183 predicted); macro F1 carried by `moderate` alone. Dordrecht's soil is ~50% real (483 BRO, 491 coord-rule). The 0.614 is not evidence of geographic generalisation; 8 of 11 features remain coordinate-based. Accuracy on the random split measures rule-recovery; cross-field validation is absent.
