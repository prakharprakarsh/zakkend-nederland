...
- **Article 15:** The model collapses on an unseen city (0.183 accuracy, below uniform random 0.225) due to a soil-type/municipality confound. A system that degenerates below random on a routine geographic change does not meet the robustness expectation. Accuracy on the random split measures rule-recovery; cross-field validation is absent.
