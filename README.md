...
These measure rule-recovery, not real-world predictive skill. Generated by
`python3 scripts/train.py` on the `hardening` branch with `RANDOM_STATE=42`.

Split strategy: **60/20/20** (6,000 train / 2,000 val / 2,000 test), early stopping
on the val set (`early_stopping_rounds=30`, best iteration: 182).

---

### Random split (stratified 60/20/20)

**Overall:** accuracy 82.5% · macro F1 0.797 · weighted F1 0.826
...
**Effect of sample weights** (same split, same random seed):
...
Sample weights improve the minority `low` class by 7.7 pp with no meaningful loss on
`moderate`.
...
**Baselines** (same 2,000-row test set):

| Model | Strategy | Accuracy |
|-------|----------|----------|
| `DummyClassifier` | `most_frequent` (always `moderate`) | 49.45% |
| `DummyClassifier` | `stratified` | 35.05% |
| XGBoost (this model) | — | **82.5%** (+33 pp over majority class) |
